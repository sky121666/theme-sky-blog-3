import{n as h}from"../../../rolldown-runtime.js?v=0.9.42&r=1a0becfc6a18";import{r as l}from"../../halo/author-card/render.js?v=0.9.42&r=1a0becfc6a18";var M=h({renderWidget:()=>N});function f(s){const n=Number(s||0);return Number.isFinite(n)&&n>0?Math.round(n):0}function $(s={}){const n=s.spec||{},a=s.status||{},c=s.metadata||{},o=String(n.displayName||c.name||"文档项目").trim(),i=String(a.permalink||"").trim(),r=f(a.totalDocs);return{title:o,description:String(n.description||"").trim(),icon:String(n.icon||"").trim(),href:i,totalDocs:r,disabled:r<=0||!i}}function m(s,n,a=""){return n?.icon?`
  ...
  `}function v(s,n,a,c){return l({href:n,app:"docsme",className:`wg-docsme wg-docsme--${c} wg-docsme--empty-projects`,attrs:`aria-label="${s("打开文档中心")}"`,disabled:a==="preview",innerHtml:`
    ...
    `})}function g({escapeHtml:s,mode:n},a){const c=a.description?`<span class="wg-docsme-row-desc">${s(a.description)}</span>`:"",o=`
  ...
  `;return a.disabled||n==="preview"?`<span class="wg-docsme-row-item is-disabled">${o}</span>`:l({href:s(a.href),app:"docsme",className:"wg-docsme-row-item",attrs:`aria-label="${s(`打开${a.title}`)}"`,innerHtml:o})}function u(s){return`
  ...
  `}function y(s,n){if(n){const a=s.find(c=>c.title===n);if(a)return a}return s.find(a=>!a.disabled)||s[0]||null}function D(s,n,a,c,o){const{escapeHtml:i,mode:r}=s,e=y(c,n?.meta?.projectTitle||""),t=e&&!e.disabled?i(e.href):a,d=e?.title||"文档中心",p=e?Math.min(100,Math.max(12,e.totalDocs/Math.max(1,o)*100)):0,w=e?`<span class="icon-[lucide--file-text]" aria-hidden="true"></span> ${e.totalDocs} Docs`:"Documentation";return l({href:t,app:"docsme",className:`wg-docsme wg-docsme--small${e?.disabled?" is-disabled":""}`,attrs:`aria-label="${i(`打开${d}`)}"`,disabled:r==="preview"||e?.disabled,innerHtml:`
    ...
    `})}function k(s,n,a,c){const{escapeHtml:o,mode:i}=s,r=a.slice(0,2),e=[];for(r.forEach(t=>{e.push(g(s,t))});e.length<2;)e.push(u(o));return`
    <div class="wg-docsme wg-docsme--medium wg-docsme-list-view">
      <span class="wg-docsme-header">
        <span class="wg-docsme-header-left">
          ...
          <strong>${o("文档中心")}</strong>
          <span class="wg-docsme-header-stats-pill" aria-label="${o(`${a.length}个项目`)}">
            <span class="icon-[lucide--layers]" aria-hidden="true"></span>
            <span>${a.length}</span>
          </span>
        </span>
        ${l({href:n,app:"docsme",className:"wg-docsme-open",attrs:`aria-label="${o("查看全部文档项目")}"`,disabled:i==="preview",innerHtml:'<span class="icon-[lucide--arrow-up-right]" aria-hidden="true"></span>'})}
      </span>
      <div class="wg-docsme-list-stack">
        ${e.join("")}
      </div>
    </div>
  `}function P(s,n,a,c){const{escapeHtml:o,mode:i}=s,r=a.slice(0,4),e=[];for(r.forEach(t=>{e.push(g(s,t))});e.length<4;)e.push(u(o));return`
    <div class="wg-docsme wg-docsme--large wg-docsme-list-view">
      <span class="wg-docsme-header">
        ...
        ${l({href:n,app:"docsme",className:"wg-docsme-view-all",attrs:`aria-label="${o("查看全部文档项目")}"`,disabled:i==="preview",innerHtml:'查看全部 <span class="icon-[lucide--chevron-right] wg-docsme-view-all-arrow" aria-hidden="true"></span>'})}
      </span>
      ...
      <span class="wg-docsme-footer">
        ...
        <span><span class="icon-[lucide--file-text]" aria-hidden="true"></span> ${c} Docs</span>
      </span>
    </div>
  `}function N(s,n){const{sources:a,escapeHtml:c,mode:o}=s;if(!a.docsmeAvailable)return b(c);const i=n?.size||"medium",r=c(a.docsmeUrl||"/docs"),e=Array.isArray(a.docsmeProjects)?a.docsmeProjects.map($).filter(d=>d.title):[],t=e.reduce((d,p)=>d+p.totalDocs,0);return e.length?i==="large"?P(s,r,e,t):i==="medium"?k(s,r,e,t):D(s,n,r,e,t):v(c,r,o,i==="large"?"large":i)}export{M as t};
