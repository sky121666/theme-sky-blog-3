import{n as N}from"../../halo/author-card/render.js";var P='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>';function U({sources:a,escapeHtml:e,mode:h},m){const v=m?.size||"small";if(!a.photosAvailable)return'<div class="desktop-widget-empty">未安装图库插件。</div>';const u=Array.isArray(a.photos)?a.photos:[],w=Array.isArray(a.photoGroups)?a.photoGroups:[],i=m?.meta?.groupName||"",t=i?u.filter(s=>s?.spec?.groupName===i):u;if(!t.length)return'<div class="desktop-widget-empty">该相册暂无照片。</div>';const b=e(a.photosUrl||"/photos"),$=i&&w.find(s=>s?.metadata?.name===i)||null,p=s=>{if($)return $;const o=s?.spec?.groupName;return o&&w.find(n=>n?.metadata?.name===o)||null},l=s=>{const o=p(s);return e(o?.spec?.displayName||o?.metadata?.name||i||"图库")},d=s=>e((p(s)?.metadata?.annotations?.description||"").trim()),f=s=>(p(s)?.metadata?.annotations?.icon||"").trim()||P,A=s=>e(s?.spec?.url||""),G=s=>e(s?.spec?.displayName||""),r=(s,o)=>{const n=A(s);return n?`<img class="${o}" src="${n}" alt="${G(s)}" loading="lazy" decoding="async" fetchpriority="low">`:`<div class="${o} is-placeholder"></div>`},y=(s,o)=>`
      <div class="${o}">
