import{i as b}from"../../catalog.js?v=0.9.30&r=de19e8d733c6";import{n as d}from"../author-card/render.js?v=0.9.30&r=de19e8d733c6";import{n as g}from"../latest-posts/render.js?v=0.9.30&r=de19e8d733c6";function M({sources:p,escapeHtml:t,mode:r},k,y={}){const w=y.compact===!0,n=k?.size||"medium";let v=3;n==="small"?v=1:n==="large"&&(v=w?4:5);const i=p.popularPosts.slice(0,v);if(!i.length)return'<div class="desktop-widget-empty">热门文章暂时为空。</div>';const $=Math.max(...i.map(s=>s?.stats?.visit??0),1);let c="";if(n==="small"){const s=i[0],o=t(s?.spec?.title||"未命名文章"),l=s?.stats?.visit??0,a=b(s?.spec?.cover,p.fallbackCover),e=a?`style="background-image:url('${t(a)}')"`:"",h=a?"":" is-no-cover";c=d({href:t(s?.status?.permalink||"#"),app:"reader",className:`wg-hot-sm${h}`,attrs:e,disabled:r==="preview",innerHtml:`
