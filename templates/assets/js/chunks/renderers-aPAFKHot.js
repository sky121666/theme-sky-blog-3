import{h as z}from"../main.js";function A(t){const s=t instanceof Date?t:new Date,e=s.getFullYear(),a=s.getMonth(),i=s.getDate(),o=new Date(e,a,1).getDay(),d=new Date(e,a+1,0).getDate(),p=["日","一","二","三","四","五","六"],c=[];p.forEach(l=>{c.push(`<span class="desktop-widget-calendar-weekday">${l}</span>`)});for(let l=0;l<o;l+=1)c.push('<span class="desktop-widget-calendar-day is-empty"></span>');for(let l=1;l<=d;l+=1){const n=l===i?" is-today":"";c.push(`<span class="desktop-widget-calendar-day${n}">${l}</span>`)}return c.join("")}function T({now:t}){const s=t.getHours(),e=t.getMinutes(),a=t.getSeconds(),i=s%12*30+e*.5,o=e*6+a*.1,d=a*6,p=Array.from({length:12},(c,l)=>{const n=l+1,r=(n*30-90)*(Math.PI/180),g=40;return`<text x="${50+g*Math.cos(r)}" y="${50+g*Math.sin(r)}" text-anchor="middle" dominant-baseline="central" class="desktop-widget-clock-number">${n}</text>`}).join("");return`
  ...
  `}function B({now:t,escapeHtml:s},e,a={}){const i=e?.size==="small",o=e?.size==="medium",d=a.preview===!0?" is-preview":"",p=`${t.getMonth()+1}月`,c=`${t.getFullYear()}年`,l=t.toLocaleDateString("zh-CN",{weekday:"long"}),n=t.toLocaleDateString("zh-CN",{day:"numeric"}),r=String(n).replace(/[^\d]/g,"")||n;return i?`
    ...
    `:o?`
      <div class="desktop-widget-calendar desktop-widget-calendar--compact${d}">
        ...
        <div class="desktop-widget-calendar-right">
          ...
          <div class="desktop-widget-calendar-grid">${A(t)}</div>
        </div>
      </div>
    `:`
    <div class="desktop-widget-calendar desktop-widget-calendar--large${d}">
      ...
      <div class="desktop-widget-calendar-grid is-large">${A(t)}</div>
    </div>
  `}function m(t){const s=Number(t||0);if(!Number.isFinite(s)||s<0)return"0";if(s<1e3)return String(Math.round(s));if(s<1e6){const a=s/1e3;return a<10?`${Math.round(a*10)/10}k`:`${Math.round(a)}k`}const e=s/1e6;return e<10?`${Math.round(e*10)/10}m`:`${Math.round(e)}m`}function y(t){const s=t?new Date(t):null;return!s||Number.isNaN(s.getTime())?"":s.toLocaleDateString("zh-CN",{month:"2-digit",day:"2-digit"}).replace("/",".")}function D(t){return Number(t?.postCount??t?.status?.postCount??t?.status?.visiblePostCount??0)||0}function j(t,s=0,e=[]){return Array.isArray(t)&&t.forEach(a=>{if(!a)return;const i=a?.metadata?.annotations||{};e.push({key:a?.metadata?.name||`category-${e.length+1}`,name:a?.spec?.displayName||a?.metadata?.name||"分类",permalink:a?.status?.permalink||"#",description:a?.spec?.description||"",cover:a?.spec?.cover||"",count:D(a),icon:i.icon||i["theme-sky-blog-3-category-setting/icon"]||"",color:i.color||i["theme-sky-blog-3-category-setting/color"]||"",depth:s}),Array.isArray(a.children)&&a.children.length&&j(a.children,s+1,e)}),e}function _(t,s){return j(t).filter(e=>e.permalink!=="#").sort((e,a)=>a.count-e.count||e.depth-a.depth||e.name.localeCompare(a.name,"zh-CN")).slice(0,Math.max(s||0,1))}function L(t,s={}){const e=t?.avatar||t?.spec?.avatar||s.logo||"";return{displayName:t?.displayName||t?.spec?.displayName||s.title||"站点作者",summary:t?.bio||t?.spec?.bio||s.subtitle||"持续发布博客内容",avatar:e,permalink:t?.permalink||s.url||"#"}}function S(t){const s=L([...Array.isArray(t?.latestPosts)?t.latestPosts:[],...Array.isArray(t?.popularPosts)?t.popularPosts:[]].map(a=>a?.owner).find(Boolean),t?.siteProfile||{}),e=t?.siteStats||{};return{...s,posts:Number(e?.post??(Array.isArray(t?.latestPosts)?t.latestPosts.length:0)??0)||0,comments:Number(e?.comment??0)||0,visits:Number(e?.visit??0)||0,moments:Array.isArray(t?.recentMoments)?t.recentMoments.length:0}}function W(t){return Number(t?.status?.visiblePostCount??t?.status?.postCount??t?.postCount??0)||0}function V(t){const s=W(t),e=t?.metadata?.annotations||{};return{name:t?.spec?.displayName||t?.metadata?.name||"标签",permalink:t?.status?.permalink||"#",count:s,icon:e["theme-sky-blog-3-tag-setting/icon"]||"",color:t?.spec?.color||""}}function I(t){let s=2166136261;const e=String(t||"");for(let a=0;a<e.length;a+=1)s^=e.charCodeAt(a),s=Math.imul(s,16777619);return s>>>0}function R(t){let s=t>>>0;return()=>(s=Math.imul(s,1664525)+1013904223>>>0,s/4294967296)}function M(t,s){const e=Array.isArray(t)?t.map(i=>V(i)).filter(i=>i.permalink!=="#"&&i.count>0):[];if(!e.length)return[];const a=R(I(`${new Date().toISOString().slice(0,10)}:${s}`));return e.map(i=>({tag:i,score:a()+Math.min(i.count,40)/100})).sort((i,o)=>o.score-i.score).slice(0,s).map(i=>i.tag).sort((i,o)=>o.count-i.count||i.name.localeCompare(o.name,"zh-CN"))}var u='data-pjax-app="reader"',$='data-pjax-app="explorer"';function E({sources:t,escapeHtml:s},e,a={}){const i=e?.size||"medium";let o=3;i==="small"?o=1:i==="large"&&(o=4);const d=t.latestPosts.slice(0,o);if(!d.length)return'<div class="desktop-widget-empty">还没有可展示的文章。</div>';let p="";if(i==="small"){const c=d[0],l=s(c?.spec?.title||"未命名文章"),n=s(y(c?.spec?.publishTime)||""),r=c?.spec?.cover||t.fallbackCover||"",g=r?`<img class="wg-news-sm-img" src="${s(r)}" alt="">`:'<div class="wg-news-sm-img is-placeholder"></div>';p=`
      <a class="wg-news-sm pjax-link" ${u} href="${s(c?.status?.permalink||"#")}">
        ${g}
    ...
    `}else if(i==="medium"){const c=d[0],l=s(c?.spec?.title||"未命名文章"),n=c?.spec?.cover||t.fallbackCover||"",r=n?`<img class="wg-news-md-img" src="${s(n)}" alt="" />`:'<div class="wg-news-md-img is-placeholder"></div>',g=d.slice(1).map(w=>{const h=s(w?.spec?.title||"未命名文章"),v=s(y(w?.spec?.publishTime)||"");return`
        <a class="wg-news-md-row pjax-link" ${u} href="${s(w?.status?.permalink||"#")}">
          <span class="wg-news-md-row-title">${h}</span>
          <span class="wg-news-md-row-date">${v}</span>
        </a>
      `}).join("");p=`
      <a class="wg-news-md-cover pjax-link" ${u} href="${s(c?.status?.permalink||"#")}">
        ${r}
      ...
      <div class="wg-news-md-body">
        <div class="wg-news-md-meta">
          ...
          <a class="wg-news-md-title pjax-link" ${u} href="${s(c?.status?.permalink||"#")}">${l}</a>
        </div>
    ...
    `}else{const c=d[0],l=s(c?.spec?.title||"未命名文章"),n=c?.spec?.cover||t.fallbackCover||"";p=`
      ...
      <div class="wg-news-lg-list">
        ${d.slice(1).map(r=>{const g=s(r?.spec?.title||"未命名文章"),w=s(y(r?.spec?.publishTime)||"");return`
        <a class="wg-news-lg-item pjax-link" ${u} href="${s(r?.status?.permalink||"#")}">
          <span class="wg-news-lg-indicator"></span>
          <span class="wg-news-lg-item-title">${g}</span>
          <span class="wg-news-lg-item-date">${w}</span>
        </a>
      `}).join("")}
        <a class="wg-news-lg-viewall pjax-link" ${$} href="${s(t.archivesUrl||"/archives")}">查看全部文章 →</a>
      </div>
    `}return`<div class="desktop-widget-news-layout is-${i}">${p}</div>`}function O({sources:t,escapeHtml:s},e,a={}){const i=a.compact===!0,o=e?.size||"medium";let d=3;o==="small"?d=1:o==="large"&&(d=i?4:5);const p=t.popularPosts.slice(0,d);if(!p.length)return'<div class="desktop-widget-empty">热门文章暂时为空。</div>';const c=Math.max(...p.map(n=>n?.stats?.visit??0),1);let l="";if(o==="small"){const n=p[0],r=s(n?.spec?.title||"未命名文章"),g=n?.stats?.visit??0,w=n?.spec?.cover||t.fallbackCover||"",h=w?`style="background-image:url('${s(w)}')"`:"";l=`
      <a class="wg-hot-sm${w?"":" is-no-cover"} pjax-link" ${u} ${h} href="${s(n?.status?.permalink||"#")}">
        ...
        <div class="wg-hot-sm-bottom">
          ...
          <span class="wg-hot-sm-visits">${s(m(g))} 阅读</span>
        </div>
      </a>
    `}else if(o==="medium")l=`
      ...
      <div class="wg-hot-md-list">${p.map((n,r)=>{const g=s(n?.spec?.title||"未命名文章"),w=n?.stats?.visit??0,h=Math.round(w/c*100);return`
        <a class="wg-hot-md-row pjax-link" ${u} href="${s(n?.status?.permalink||"#")}">
          ...
          <div class="wg-hot-md-info">
            ...
            <div class="wg-hot-md-bar"><span style="width:${h}%"></span></div>
          </div>
          <span class="wg-hot-md-visits">${s(m(w))}</span>
        </a>
      `}).join("")}</div>
    `;else{const n=p[0],r=s(n?.spec?.title||"未命名文章"),g=n?.spec?.cover||t.fallbackCover||"",w=n?.stats?.visit??0,h=g?`<img class="wg-hot-lg-cover-img" src="${s(g)}" alt="" />`:'<div class="wg-hot-lg-cover-img is-placeholder"></div>',v=p.slice(1).map((k,f)=>{const N=s(k?.spec?.title||"未命名文章"),b=k?.stats?.visit??0,P=Math.round(b/c*100);return`
        <a class="wg-hot-lg-row pjax-link" ${u} href="${s(k?.status?.permalink||"#")}">
          <span class="wg-hot-lg-rank${f<2?" is-top":""}">${String(f+2).padStart(2,"0")}</span>
          <div class="wg-hot-lg-info">
            <span class="wg-hot-lg-row-title">${N}</span>
            <div class="wg-hot-lg-bar"><span style="width:${P}%"></span></div>
          </div>
          <span class="wg-hot-lg-visits">${s(m(b))}</span>
        </a>
      `}).join("");l=`
      <div class="wg-hot-lg-hero">
        <a class="wg-hot-lg-cover pjax-link" ${u} href="${s(n?.status?.permalink||"#")}">
          ${h}
          ...
          <div class="wg-hot-lg-cover-text">
            ...
            <strong>${r}</strong>
            <span class="wg-hot-lg-hero-visits">${s(m(w))} 阅读</span>
          </div>
        </a>
      </div>
      <div class="wg-hot-lg-list">${v}</div>
    `}return`<div class="${`wg-hot-wrap wg-hot-wrap--${o}${i?" is-compact":""}`}">${l}</div>`}var F='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" fill="currentColor"><path d="M216,72H131.31L104,44.69A15.86,15.86,0,0,0,92.69,40H40A16,16,0,0,0,24,56V200.62A15.4,15.4,0,0,0,39.38,216H216.89A15.13,15.13,0,0,0,232,200.89V88A16,16,0,0,0,216,72ZM40,56H92.69l16,16H40Z"/></svg>';function Z({sources:t,escapeHtml:s}){const e=_(t.categories,4);if(!e.length)return'<div class="desktop-widget-empty">当前没有可展示的分类。</div>';const a="/categories",i=e.map(o=>{const d=o.color||"currentColor",p=o.icon||F,c=d!=="currentColor"?` style="color:${s(d)}"`:"";return`
      <a class="wg-cat-item pjax-link" ${$} href="${s(o.permalink)}">
    ...
    <div class="wg-cat-obsidian">
      <div class="wg-cat-header">
        ...
        <a class="wg-cat-more pjax-link" ${$} href="${s(a)}">
          更多分类
    ...
    </div>`}function U({sources:t,escapeHtml:s},e){const a=S(t),i=s(a.permalink||"#"),o=a.avatar?`<img class="wg-author-avatar-img" src="${s(a.avatar)}" alt="${s(a.displayName)}">`:`<span class="wg-author-avatar-fallback">${s((a.displayName||"A").slice(0,1))}</span>`,d=i;return`
    <div class="wg-author-compact">
      <a class="wg-author-head" href="${i}">
  ...
  `}function X({modules:t,sources:s,escapeHtml:e},a){const i=s.siteStats;if(!i)return'<div class="desktop-widget-empty">站点统计当前不可用。</div>';const o='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg>',d='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>',p='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z"/><path d="M14 2v6h6"/><path d="M16 13H8"/><path d="M16 17H8"/><path d="M10 9H8"/></svg>',c='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"/></svg>',l='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20V10"/><path d="M18 20V4"/><path d="M6 20v-4"/></svg>';if(a.size==="medium")return`
      ...
      <div class="wg-stat-ring">
        ...
        <div class="wg-stat-ring-chart-wrap">
          <svg viewBox="0 0 100 100" class="wg-stat-ring-svg">
            ...
            <circle cx="50" cy="50" r="40" fill="none" stroke="#f43f5e" stroke-width="14" stroke-dasharray="${v}" stroke-dashoffset="${v*(1-h)}" stroke-linecap="round" style="transform-origin: center; transform: rotate(${360*w}deg);"/>
          </svg>
          <div class="wg-stat-ring-center">
            <span class="wg-stat-ring-total">${e(m(g))}</span>
          </div>
        </div>
    ...
    `}return a.size==="extra-large"?`
      <div class="wg-stat-hero-solo">
        ...
        <div class="wg-stat-hero-solo-bottom">
          <h3>总访问量</h3>
          <div class="wg-stat-hero-solo-value glow-text-emerald">${e(m(i.visit??0))}</div>
        </div>
      </div>
    `:`
    <div class="wg-stat-hero-list">
      <div class="wg-stat-hero">
        ...
        <div class="wg-stat-hero-value glow-text-emerald">${e(m(i.visit??0))}</div>
      </div>
      <div class="wg-stat-list">
        <div class="wg-stat-list-item">
          ...
          <span class="wg-stat-list-value">${e(m(i.post??0))}</span>
        </div>
        <div class="wg-stat-list-item">
          <div class="wg-stat-list-label"><span class="wg-stat-list-icon color-rose">${c}</span><span>评论互动</span></div>
          <span class="wg-stat-list-value">${e(m(i.comment??0))}</span>
        </div>
      </div>
    </div>
  `}function x(t,s=0){let e=s;for(let a=0;a<t.length;a++)e=(e<<5)-e+t.charCodeAt(a)|0;return(e&2147483647)%1e3/1e3}var C=[{top:"12%",left:"8%"},{top:"72%",left:"80%"},{top:"38%",left:"12%"},{top:"18%",left:"75%"},{top:"65%",left:"18%"},{top:"45%",left:"82%"},{top:"82%",left:"40%"}];function Y({sources:t,escapeHtml:s},e,a={}){const i=a.compact===!0,o=e?.size||"medium";if(o==="small"){const n=M(t.randomTags,8);return n.length?`<div class="wg-tag-focus-stage" data-tag-focus>${n.map((r,g)=>{const w=C[g%C.length],h=Object.entries(w).map(([k,f])=>`${k}:${f}`).join(";"),v=r.color||"#A1A1AA";return`<a class="${g===0?"wg-tag-focus-item is-focus":"wg-tag-focus-item"} pjax-link" ${$} href="${s(r.permalink)}" style="${h};--tag-color:${s(v)}">${s(r.name)}</a>`}).join("")}</div>`:'<div class="desktop-widget-empty">无标签</div>'}const d=i?10:12,p=M(t.randomTags,d);if(!p.length)return'<div class="desktop-widget-empty">当前没有可展示的标签。</div>';const c=(n,r,g="")=>{const w=n.color||"#A1A1AA",h=((x(n.name,1)-.5)*5).toFixed(1),v=((x(n.name,2)-.5)*3.2).toFixed(1),k=((x(n.name,3)-.5)*3.2).toFixed(1);return`
      <a class="wg-tag-chip ${g} tone-${r%4} pjax-link"
         ${$}
         href="${s(n.permalink)}"
         title="${s(n.name)}"
         style="--tag-color:${s(w)};--j-rot:${h}deg;--j-tx:${v}px;--j-ty:${k}px;">
        ${s(n.name)}
      </a>
    `};let l="";if(o==="large"){const n=p[0],r=p.slice(1),g=n?`
      <a class="wg-tag-feature pjax-link"
         ${$}
         href="${s(n.permalink)}"
         style="--tag-color:${s(n.color||"#A1A1AA")}">
        <span class="wg-tag-feature-kicker">探索</span>
        <strong>${s(n.name)}</strong>
      </a>
    `:"",w=r.map((h,v)=>c(h,v)).join("");l=`
      <div class="wg-tag-wall-shell is-large${i?" is-compact":""}">
        ${g}
        <div class="wg-tag-wall is-large${i?" is-compact":""}">${w}</div>
      </div>
    `}else{const n=p.map((r,g)=>c(r,g)).join("");l=`<div class="wg-tag-wall is-medium${i?" is-compact":""}">${n}</div>`}return l}typeof document<"u"&&setInterval(()=>{document.querySelectorAll("[data-tag-focus]").forEach(t=>{const s=t.querySelectorAll(".wg-tag-focus-item");if(s.length<2)return;let e=-1;s.forEach((i,o)=>{i.classList.contains("is-focus")&&(e=o)});const a=(e+1)%s.length;s.forEach((i,o)=>{i.classList.toggle("is-focus",o===a)})})},4e3);function q({sources:t,escapeHtml:s,normalizeMomentRecord:e},a){if(!t.momentsAvailable)return'<div class="desktop-widget-empty">未安装 Moments 插件。</div>';const i=t.recentMoments.slice(0,1).map(n=>e(n));if(!i.length)return'<div class="desktop-widget-empty">还没有可展示的瞬间。</div>';const o=i[0],d=S(t),p=o.tags.length>0?o.tags[0]:"",c=t.recentMoments[0]?.stats?.upvote??0,l=t.recentMoments[0]?.stats?.totalComment??0;return`
    <a class="wg-moment-social pjax-link" data-pjax-app="moments-app" href="${s(o.permalink)}">
  ...
  `}function J(t,s){const e=s?.data;return e||{city:t.weather.cityName||"天气",temperature:"--",high:"--",low:"--",apparent:"--",humidity:"--",windSpeed:"--",condition:"等待数据",icon:"☁︎",tone:"cloudy",updatedAt:null}}function K(t,s){return`
  ...
  `}function H({modules:t,weatherState:s,escapeHtml:e},a,i={}){const o=t.weather.cityName,d=a?.size==="small";if(!o)return'<div class="desktop-widget-empty">请先在后台设置天气组件城市。</div>';if(s.loading&&!s.data)return d?ss():ts();const p=J(t,s);return d?K(p,e):Q(p,e)}function ss(){return`
  ...
  `}var is=z({renderDesktopWidget:()=>es});function es(t,s,e={}){switch(s.widget){case"system.clock":return T(t,s,e);case"system.calendar":return B(t,s,e);case"system.weather":return H(t,s,e);case"halo.latest_posts":return E(t,s,e);case"halo.popular_posts":return O(t,s,e);case"halo.categories":return Z(t,s,e);case"halo.author_card":return U(t,s,e);case"halo.site_stats":return X(t,s,e);case"halo.random_tags":return Y(t,s,e);case"plugin-moments.recent":return q(t,s,e);default:return G(t.escapeHtml,s)}}export{j as n,is as t};
