...
function isSameOriginOrRelativeUrl(value) {
...
export function resolveWidgetCover(primaryCover, fallbackCover = '') {
  const primary = String(primaryCover || '').trim();
  if (primary) {
    return primary;
  }

  const fallback = String(fallbackCover || '').trim();
  if (!fallback) return '';

  return isSameOriginOrRelativeUrl(fallback) ? fallback : '';
}
