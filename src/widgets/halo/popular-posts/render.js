...
export function renderWidget({ sources, escapeHtml, mode }, widget, options = {}) {
  ...
  const limit = defaultLimitForSize(size, isCompact);
  ...
  if (size === 'small') {
    ...
    const rowsHTML = posts.map((post, i) => {
      ...
      const secondary = escapeHtml(formatCompactNumber(visit));
    ...
    const heroSecondary = `${escapeHtml(formatCompactNumber(heroVisit))} 阅读`;
