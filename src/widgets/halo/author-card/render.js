...
export function renderWidget({ sources, escapeHtml, mode }) {
  ...
  const authorApp = author.authenticated ? 'auth' : 'explorer-author';
  ...
  const postsApp = author.authenticated ? 'auth' : 'explorer-author';
  ...
  return `
    <div class="wg-author-compact" data-author-authenticated="${author.authenticated ? 'true' : 'false'}">
      ${buildWidgetPjaxLink({
        href: authorHref,
        app: authorApp,
      ...
      <div class="wg-author-actions">
        ${buildWidgetPjaxLink({ href: postsHref, app: postsApp, className: 'wg-author-action-btn', attrs: author.authenticated ? 'title="个人中心"' : 'title="文章"', disabled: mode === 'preview', innerHtml: postsSvg })}
