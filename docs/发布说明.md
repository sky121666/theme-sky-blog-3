...
- 全部直接依赖升级并锁定到本轮最新稳定版；切换为 Node `24.18.0`、pnpm `11.15.0`、Vite `8.1.5`、Tailwind CSS `4.3.3` 与官方 Vite 插件。
...
- pnpm 开启严格引擎、构建脚本白名单、最小发布时间、信任策略与冻结锁文件约束；18 个“最新稳定版但未满 7 天”的精确版本例外均有理由、owner 与 2026-07-27 到期门禁；CI/CD Action 固定到提交 SHA。
- 发布包新增许可证、第三方声明、版本一致性、路径穿越、重复/大小写冲突、敏感目录与 SHA-256 校验。
- CD 只允许 `main` 可达提交上的标签；先完成构建和双 Halo 回归，再创建 GitHub 草稿并同步应用市场，同步成功后才公开 GitHub Release。应用市场 Action 失败可能留下 Halo 草稿；此时 GitHub 草稿作为安全重跑锁保留，必须先手工清理 Halo 草稿，再删除 GitHub 草稿后重跑。若应用市场已成功、仅 GitHub Release 公开失败，工作流会进行最多 5 次有界尝试；仍失败时只能直接公开既有 GitHub 草稿，禁止删除草稿或重跑 CD。
- 发布前置条件新增 `HALO_PAT`“版本管理”权限检查，并由静态契约保证 `theme.yaml` 的 `store.halo.run/app-id` 与 CD App Store Action 的 `app-id` 一致（当前为 `app-gqnoxtpt`）。
- 新增发布与回滚契约：公开新版本前必须验证上一稳定 Release 的 ZIP；已发布版本不可覆盖，严重回归恢复上一稳定 ZIP并发布更高补丁版本，工作流禁止自动删除标签或公开版本。

### 兼容性

- Halo 最低版本提高到 `>=2.25.0`。
- 隔离 Halo `2.25.0` 已完成主题包安装、启用、首页协议与 Playwright Core 回归；本机 Halo `2.25.4` 已完成 13 个 reload 路由和 22 个 Playwright 场景。
...
- `node scripts/verify-release-package.mjs dist/theme-sky-blog-3-0.9.41.zip`
