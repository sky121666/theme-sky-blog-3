...
assert.match(detailTemplate, /#annotations\.getOrDefault\(item, 'icon', ''\)/, '标签列表必须读取标签图标元数据');
assert.match(detailTemplate, /item\.spec\.color/, '标签列表必须读取 Halo 原生标签颜色');
assert.match(detailTemplate, /data-tag-custom-icon/, '标签列表必须暴露自定义图标渲染状态');
assert.match(detailTemplate, /data-tag-color/, '标签列表必须暴露通过校验的颜色值');
assert.match(detailTemplate, /tags-folder-icon--custom/, '标签列表必须渲染自定义 SVG 图标');
assert.match(detailTemplate, /icon-\[lucide--tag\]/, '标签缺少图标元数据时必须回退默认标签图标');
