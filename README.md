...
- [ ] 桌面小组件（重中之重）
  - 已接入独立 `widgets` 设置分类、默认实例和首页桌面层
  - 已支持编辑态、拖拽吸附预览、添加组件、隐藏组件和恢复默认
  - 已接入组件中心、扩展类型（天气 / 站点统计 / 随机标签 / 瞬间）和 `extension` 存储桥
  - 当前布局已写入浏览器本地存储，后续插件只负责用户布局同步
  - 下一步是补真实用户同步插件和更多组件类型
...
- [x] 桌面小组件 MVP
  ...
  - 已接入 `local` 和 `extension` 双存储驱动边界
  - 未来插件只需要实现用户布局同步，不接管组件 UI
