...
| 友链与朋友圈 | `/links`、分组、RSS 朋友圈、留言板、权限感知添加助手 | 已完成 |
...
| 朋友圈 `plugin-friends.recent` | `PluginLinks` RSS | medium / large |
...
| `PluginLinks` | `2.2.1` | 友链列表、分组、RSS 朋友圈、游标续载、留言板、权限感知识别与添加、桌面朋友圈小组件 |
...
- PluginLinks 未开启公开 RSS 时，朋友圈显示明确空态；不会回退到已经退出主题契约的 `plugin-friends`。
