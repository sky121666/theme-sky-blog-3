...
- [ ] 桌面小组件（重中之重）
  ...
  - 已支持基于 `@alpinejs/morph` 的增量内容更新，避免时钟、天气刷新时的 DOM 闪烁
  - 已补齐渲染器 HTML 缓存，支持按 `appearance` 契约精准失效
  - 已修复编辑态下组件内部链接抢占点击的问题，优先保证桌面编排交互
...
- [x] Explorer 目录页手机端沉浸式适配（tags / categories / archives）
  - `@media (max-width: 768px)` 隐藏 menubar、Dock、交通灯、缩放手柄
  - 窗口全屏化：`100dvh` + `border-radius: 0` + `transform: none`
  - 内容层压缩：sidebar nav grid、folder card grid、post-row 元信息换行、分页紧凑化
  - `:has()` + `:not(.author-shell)` 精确排除 author 页，避免误伤

- [x] Author 页手机端沉浸式适配 + browser 全局 mobile 边界统一
  - browser window `isDesktop` 阈值从 `>=768` 改为 `>768`，CSS 断点 `767px→768px`
  - browser 返回按钮从 reader 专属改为 mobile 全 browser 通用 `__browserBackOrHome`
  - author 引入 `previewVisible` / previewless 双模式：resize 自动检测、选中态弱化、path 文案切换
  - author 768px 沉浸块：资料卡压缩、source 2 列、posts/moments 换行、分页紧凑
  - destroy() 清理 resize 监听，防止 PJAX 累积

- [x] Header submenu 继续细化
  - dropdown 收紧至 184px / 8px 圆角 / 短投影
  - 三列 grid 补齐显式尺寸：leading 16px / label 省略 / trailing 12px
  - hover 从双色渐变改为单色 accent fill + focus-visible ring
  - separator 起点对齐 label 列（margin-left: 30px）
  - 顶栏背景 light/dark 分离：浅灰透明 / 近全透，移除描边

- [x] Dock 细化 + 后台可配置
  - 最小化窗口图标改为语义类 `dock-minimized-thumb` + dark 模式 SVG 自适应
  - 分隔线从 Tailwind → `dock-separator` 语义类，`align-self: flex-end` 底对齐
  - glass 材质升级：blur 60px / saturate 180% / 0.5px border / 多层阴影
  - light/dark glass 分离：白色透明 vs 深色透明，surface.css 变量独立
  - 放大引擎升级：cos² → cos³ 衰减、range/lift/maxSize 基于 icon_size 动态计算
  - Genie 动画：420→480ms、easing 曲线更柔和
  - dock-dot 改 white glow 替代 accent 色
  - AbortController + destroy() 清理 PJAX 监听泄漏
  - 新增 6 个后台设置项：图标大小 / 图标间距 / 内边距 / 放大倍率 / 背景模糊 / 背景透明度
  - 数据流：settings.yaml → data-dock-* → JS clamp+setProperty → CSS var(--dock-*)

- [x] 桌面图标与小组件样式收口
  - 删除冗余 `.selected` / `.dark` 重复选中态声明（-28 行）
  - 图标 label text-shadow 0.62→0.42，浅色壁纸更自然
  - light 模式小组件 border 0.08→0.05，接近 macOS 无边框质感

- [x] 站内导航性能优化
  - 同变体内容级切换：`explorer→explorer`、`moments→moments`、`photos→photos` 不重建窗口框架
...
- [x] 瞬间 (Moments) 独立窗口
  ...
  - 已支持基于 Alpine.js x-intersect 的无感瀑布流加载与详情页返回精确滚动坐标自动恢复机制
  ...
  - 移动端沉浸式全屏布局，微信朋友圈风格封面区 + 浮动透明标题栏
  - feed 滚动超过封面后标题栏自动切换为实色（亮 `#e0e0e0` / 暗 `#222`），0.25s 平滑过渡
  - 详情页标题栏自动隐藏 feed 按钮（刷新/切换外观），纯 CSS `:has()` 响应场景切换
  - 同变体导航 frame 层场景同步：标题文本、按钮可见性、返回 fallback、scroll 监听均在 post-swap 中更新
  - 返回按钮使用 `document.referrer` 同源检查，外部来源和冷启动自动回退站内路径
  - 同变体点击拦截提升至 capture phase + `stopImmediatePropagation`，修复详情页双重加载
...
- [x] Header 顶栏与二级菜单
  ...
  - 顶栏已拆分为独立 `modules/header.html`，支持桌面菜单与手机端菜单共用协议
  - 后台已新增 Header 搜索 / 登录 / 手机菜单开关，以及桌面端 / 手机端时间格式方案
  - Header 已支持独立登录入口文案和 `/login` 跳转控制，Cmd/Ctrl + K 也会跟随搜索开关
  - 已新增 auth gateway 独立布局与样式入口，登录类页面不再复用桌面壳层
  - 已建立 Header 固定图标兼容层：默认内置 Lucide SVG + 后台 `iconify(svg)` 可覆盖
...
- [x] 图库（Photos）独立窗口
  - 已接入独立 `photos-app` Vite 入口（`photos-app.css` / `photos-app.js`）
  - 已补齐完整 macOS 标题栏协议：交通灯 + 返回导航 + 标题/副标题 + 明暗切换 + 刷新按钮
  - 标题栏工具区已迁入 `photos.html` window frame，移除原内容区内嵌 header 控件
  - 已支持方格（Square Grid）与瀑布流（Masonry）双布局模式
  - 布局切换采用持久化分布状态（`masonryDistribution`），切换回 `aspect` 时恢复原有列分布，不触发重排
  - `squareCols / aspectCols / albumCols` 三套列数独立管理，通过 `localStorage` 持久化
  - Photos 已加入 PJAX 同变体白名单（`photos-app → browser-list`），相簿/分组切换保留布局和滚动位置
  - 图片懒加载已接入共享 `initLazyImages`（IntersectionObserver），移除原 `hydrateImage` 强制预加载
  - 无限滚动新增加载骨架屏（8 张随机高度占位卡），追加卡片直接落入最短列
  - `destroy()` 已完整清理：IntersectionObserver / ResizeObserver / resize handler / heightSyncFrame / 骨架卡 / body overflow
  - 精选集 (Albums) 视图已改为单张满幅封面 + 渐变覆盖文字，去除叠层伪元素堆叠效果
  - 完整 CSS 设计 token 体系（亮/暗），覆盖窗口阴影、标题栏材质、侧栏、封面、骨架屏、灯箱全部组件
  - 移动端沉浸式适配：`data-page-app="photos-app"` 选择器精确隔离，菜单栏/Dock/交通灯隐藏，全屏化窗口
...
- [x] 文章页阅读模式
  ...
  - 已补齐桌面端吸附目录、窄窗口折叠目录，以及移动端标题栏目录按钮 + 底部目录面板
